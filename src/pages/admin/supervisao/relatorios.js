...
function PrintBar({ item, color, formatter = formatMetricPercent }) {
    ...
    <div className="supervisao-print-report">
      ...
      <div className="print-section">
        ...
        >
          ...
          <div className="print-metric-card">
            ...
            <strong>{formatMetricDecimal(metrics.competencia)}</strong>
      ...
      <div className="print-section">
        ...
        <h3 style={{ marginTop: "40px" }}>
          Gestão de planos de ação em aberto
        </h3>
          ...
          <table className="print-table">
            ...
            <tbody>
              {planosPendentes.map((plano, index) => (
                <tr key={plano.id || `plano-${index}`}>
                  <td>
                    <strong>
                      {plano.prazo
                        ? formatarDataBR(plano.prazo)
                        : "Sem prazo"}
                    </strong>
                  </td>
                  ...
                  <td>
                    {safeText(
                      plano.planoAcao || plano.recomendacao,
                      "Sem detalhamento"
                    )}
                  </td>
                </tr>
        ...
        {recomendacoes.length ? (
          ...
          >
            {recomendacoes.map((item, index) => (
              <div
                key={item.id || `recomendacao-${index}`}
                style={{
                  background: "#fff",
                  border: "1px solid #e8ddd3",
                  borderRadius: "12px",
          ...
          <table className="print-table">
            <thead>
              <tr>
                ...
                <th style={{ width: "60%" }}>Resumo/motivo</th>
              </tr>
            </thead>

            <tbody>
              {previewAlertas.map((alerta, index) => (
                <tr key={alerta.id || `alerta-${index}`}>
                  <td>
...
function RelatoriosContent({ user, access, onLogout }) {
  ...
  const selectedPatient = useMemo(
    () =>
      pacientes.find(
        (item) => safeId(item?.id) === safeId(filters.pacienteId)
      ) || null,
    [pacientes, filters.pacienteId]
  );

  const selectedTherapistId =
    filters.terapeutaId || selectedPatient?.terapeutaId || "";

  const selectedTherapist = useMemo(
    () =>
      terapeutas.find(
        (item) => safeId(item?.id) === safeId(selectedTherapistId)
      ) || null,
    [terapeutas, selectedTherapistId]
  );
  ...
  const clinicasFiltradas = useMemo(() => {
    ...
    return clinicas.filter(
      (item) => safeId(item?.id) === safeId(selectedClinicId)
    );
  ...
  const terapeutasFiltrados = useMemo(() => {
    return terapeutas.filter((item) => {
      if (
        selectedClinicId &&
        safeId(item?.clinicaId) !== safeId(selectedClinicId)
      ) {
        return false;
      }
      ...
      if (
        selectedTherapistId &&
        safeId(item?.id) !== safeId(selectedTherapistId)
      ) {
        return false;
      }
  ...
  const pacientesFiltrados = useMemo(() => {
    return pacientes.filter((item) => {
      ...
      if (
        selectedTherapistId &&
        safeId(item?.terapeutaId) !== safeId(selectedTherapistId)
      ) {
        return false;
      }
      ...
      if (
        filters.pacienteId &&
        safeId(item?.id) !== safeId(filters.pacienteId)
      ) {
        return false;
      }
  ...
  const rankingMode = selectedClinicId
    ? "terapeutas"
    : "clinicas";
  ...
  const reportRows = useMemo(() => {
    ...
    return {
      ...
      terapeutasRows: buildTerapeutasRows(
        terapeutasFiltrados,
        context
      ),
      pacientesRows: buildPacientesRows(
        pacientesFiltrados,
        context
      ),
      lancamentosRows: buildLancamentosRows(
        lancamentosFiltrados,
        context
      ),
          ...
          <>
            ...
            <section className="supervisao-indicator-grid executive">
              ...
              <CardIndicador
                ...
                value={formatMetricPercent(metrics.evolucao)}
                detail="somente indicadores computados"
              />

              <CardIndicador
                label="Adesão às tarefas"
                value={formatMetricPercent(metrics.adesao)}
            ...
            <div className="bento-grid dashboard-lower">
              ...
              <div className="bento-col bento-6">
                <PreviewTable
                  ...
                  rows={reportRows.lancamentosRows}
                  limit={4}
                />
              </div>
            </div>
